...
root1 = r'D:\PJ1data\apre\T2_train'
root2 = r'D:\PJ1data\apre\T2_test'
...
writer = SummaryWriter("../logs_train_T2_pre")
best_acc = 0
for epoch in range(num_epochs):
    ...
    if test_acc > best_acc:
        torch.save(model.state_dict(), "checkpoints/T2/model-best_T2_pre.pth")
        best_acc = test_acc
    torch.save(model.state_dict(), "checkpoints/T2/model-best_T2_pre.pth")
...
def makedir(path):
    if not os.path.isdir(path):
        os.makedirs(path)
makedir('./checkpoints/T2')
# 加载预训练模型，这里假设您的模型名为 MyModel，并保存在 pth 文件中
class MyModel(nn.Module):
    def __init__(self):
        super(MyModel, self).__init__()
...
# 加载预训练模型
pretrained_dict = torch.load('./checkpoints/T2/model-best_T2_pre.pth')
model_dict = model.state_dict()
pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
model_dict.update(pretrained_dict)
model.load_state_dict(model_dict)
...
# 训练模型
timer, num_batches = d2l.Timer(), len(train_iter)
writer = SummaryWriter("../logs_train_T2_last")
best_acc = 0
for epoch in range(num_epochs):
    print(f'epoch:{epoch}')
    metric = d2l.Accumulator(3)
    ...
    if test_acc > best_acc:
        torch.save(model.state_dict(), "checkpoints/T2/model-best_T2_last.pth")
        best_acc = test_acc
    torch.save(model.state_dict(), "checkpoints/T2/model-best_T2_last.pth")
    print(f'loss {train_loss:.3f}, train acc {train_acc:.3f}, '
          f'test acc {test_acc:.3f}')
